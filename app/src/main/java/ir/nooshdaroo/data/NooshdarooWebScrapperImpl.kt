...
class NooshdarooWebScrapperImpl(
    private val fetcher: NooshdarooWebFetcher
) : NooshdarooWebScrapper {

    override suspend fun extractContentCategoriesWithPath(): List<Category> {
        return nooshdarooDoc {
            div {
                ...
                div {
                    ...
                    findFirst {
                        eachLink.entries.map { (text, url) ->
                            Category(text, Url("$BASE_URL$url"))
                        }
                    }
                }
            }
    ...
    private suspend fun <T> nooshdarooDoc(init: Doc.() -> T): T {
        return withContext(Dispatchers.Default) {
            htmlDocument(fetcher.fetchContent(), init = init)
        }
    }
}
